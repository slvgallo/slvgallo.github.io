...
fetch("data/works.json")
  ...
  .then((works) => {
    ...
    if (initialFilter) {
      ...
      const hasTag = works.some(work => work.tags && work.tags.includes(initialFilter));
    ...
    // クリックイベントの委譲（状態保存用）は初期化直後に実行されるためここでは不要
  ...
  grid.addEventListener("click", (e) => {
    ...
    if (link && link.href && (link.href.includes("works.html") || link.href.includes("/works/"))) {
      const state = {
        ...
        displayedCount: displayedCount || document.querySelectorAll("#works-grid .post").length
      };
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", setupStateSaving);
} else {
  setupStateSaving();
}
